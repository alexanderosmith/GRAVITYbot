#####################################################################################################
# DOCUMENTATION NOTES : #############################################################################
# File Creator: Alexander O. Smith (Spring 2024-August 2024), aosmith@syr.edu
...
#####################################################################################################
# Prioritized Backlog:
# 1. Find a way to provide source URLs for the GRAVITYbot response
# 2. Create Functions to deal with aLog imported data.
#   a. (In _test_alog.py) scrape aLog data
#   b. Import aLog data here (see import modules)
#   c. Create function(s) that handle aLog cleaining
# 3. Prompt modifications:
#   a. Ask whether there are problems with prior classificaitons
#   b. Conversations about detector-specific influence in glitch detection
#   c. Consider hashtags in the prompt (Carsten)
#   d. Consider multiple prompts for multiple purposes, and whether they should be automated
# 
# Optional Future Considerations:
# 1. Find a way to automate Talk imports using Panoptes API (See ./_data/talk_data.py)
#   a. So far Talk forum data must be imported manually using Panoptes python client
#   b. ???
# 2. For LIGO papers, search the PDFs or published acknoledgements for required NSF ADS statements
# DEPENDENCIES ######################################################################################
...
# ChatGPT Functions: sends prompts to chatGPT (NOTICE: these cost the money)
def chat_with_gpt3(user_prompt, sys_prompt):
    response = client.chat.completions.create(
        ...
        model="gpt-3.5-turbo-0125",                      # The openAI model for the project
        temperature=0.8,                                 # Lower = more flexibility, Higher = more accurate
        max_tokens=1500                                  # NOTICE: higher tokens, more money.
    )
...
def chat_with_gpt4(user_prompt, sys_prompt):
    ...
    with open("_output/test_gravityBot_output.txt", "a") as out_file:
...
#####################################################################################################
# SCRATCH WORK: Do not include in __main__.py #######################################################
#####################################################################################################
